...
const getOverdueInfo = (task: any, now: Date) => {
  const warnings: any[] = [];
  let isOverdue = false;
  let daysOverdue = 0;
  let daysRemaining = 0;
...
const getSourceInfo = async (sourceType: string, sourceId: string) => {
  switch (sourceType) {
    case 'CASE': {
      const c = await prisma.case.findUnique({ where: { id: sourceId }, select: { caseNumber: true, title: true } });
      return { sourceNumber: c?.caseNumber, sourceName: c?.title };
    }
    case 'CLUE': {
      const c = await prisma.clue.findUnique({ where: { id: sourceId }, select: { clueNumber: true, title: true } });
      return { sourceNumber: c?.clueNumber, sourceName: c?.title };
    }
    case 'EVIDENCE': {
      const e = await prisma.evidence.findUnique({ where: { id: sourceId }, select: { evidenceNumber: true, name: true } });
      return { sourceNumber: e?.evidenceNumber, sourceName: e?.name };
    }
    case 'PERSON': {
      const p = await prisma.person.findUnique({ where: { id: sourceId }, select: { name: true } });
...
export default async function (fastify: FastifyInstance) {
  fastify.get('/tasks', async (request: FastifyRequest<{ Querystring: TaskQuery }>, reply) => {
    const {
      page = 1,
      pageSize = 10,
      keyword,
    ...
    const [items, total] = await Promise.all([
      ...
      prisma.commandTask.count({ where }),
    ]);
    ...
    if (onlyOverdue === 'true') {
      enrichedItems = enrichedItems.filter((i: any) => i.isOverdue);
    }
    if (onlyWarning === 'true') {
      enrichedItems = enrichedItems.filter((i: any) => i.hasWarning);
    }
  ...
  fastify.get('/tasks/stats', async () => {
    const now = new Date();
    const allTasks = await prisma.commandTask.findMany({
      include: { _count: { select: { progresses: true } } },
    });

    const stats = {
      total: allTasks.length,
      byStatus: { PENDING: 0, IN_PROGRESS: 0, COMPLETED: 0, CANCELLED: 0, OVERDUE: 0 } as any,
    ...
    allTasks.forEach((task: any) => {
      ...
      if (warnings.some((w: any) => w.type === 'nearDue')) stats.nearDueCount++;
      if (warnings.length > 0) stats.warningCount++;
    });
  ...
  fastify.get('/tasks/warnings', async () => {
    ...
    allWarnings.sort((a, b) => {
      const levelOrder: any = { danger: 0, warning: 1 };
  ...
  fastify.get('/tasks/:id', async (request: FastifyRequest<{ Params: { id: string } }>, reply) => {
    const task = await prisma.commandTask.findUnique({
  ...
  fastify.post('/tasks', async (request: FastifyRequest<{ Body: TaskCreate }>, reply) => {
    const data = request.body;
    const count = await prisma.commandTask.count();
  ...
  fastify.put('/tasks/:id', async (request: FastifyRequest<{ Params: { id: string }; Body: TaskUpdate }>, reply) => {
    const data = request.body;
    try {
      const beforeTask = await prisma.commandTask.findUnique({ where: { id: request.params.id } });
      ...
      const task = await prisma.commandTask.update({
        where: { id: request.params.id },
        data: updateData,
      });
      ...
      await logUpdate(
        TargetType.COMMAND_TASK,
        task.id,
        `更新任务：${task.taskNumber} - ${task.title}`,
        request,
        {
  ...
  fastify.delete('/tasks/:id', async (request: FastifyRequest<{ Params: { id: string } }>, reply) => {
    try {
      const beforeTask = await prisma.commandTask.findUnique({ where: { id: request.params.id } });

      await prisma.$transaction([
        prisma.taskProgress.deleteMany({ where: { taskId: request.params.id } }),
        prisma.flowRecord.deleteMany({ where: { taskId: request.params.id } }),
        prisma.commandTask.delete({ where: { id: request.params.id } }),
      ]);

      await logDelete(
        TargetType.COMMAND_TASK,
        request.params.id,
        `删除任务：${beforeTask?.taskNumber || ''} - ${beforeTask?.title || ''}`,
        request,
        beforeTask ? {
          id: beforeTask.id,
  ...
  fastify.post('/tasks/:id/assign', async (request: FastifyRequest<{ Params: { id: string }; Body: TaskAssign }>, reply) => {
    try {
      const task = await prisma.commandTask.findUnique({ where: { id: request.params.id } });
      if (!task) {
        reply.status(404).send({ error: '任务不存在' });
        return;
      }
      ...
      const updated = await prisma.commandTask.update({
        where: { id: request.params.id },
        data: {
  ...
  fastify.post('/tasks/:id/transfer', async (request: FastifyRequest<{ Params: { id: string }; Body: TaskTransfer }>, reply) => {
    try {
      const task = await prisma.commandTask.findUnique({ where: { id: request.params.id } });
      if (!task) {
        reply.status(404).send({ error: '任务不存在' });
      ...
      await createOperationLog({
        targetType: TargetType.COMMAND_TASK,
        targetId: request.params.id,
        action: ActionType.TASK_TRANSFER,
  ...
  fastify.post('/tasks/:id/complete', async (request: FastifyRequest<{ Params: { id: string }; Body: { result?: string; note?: string; operatorName?: string } }>, reply) => {
    try {
      const task = await prisma.commandTask.findUnique({ where: { id: request.params.id } });
      if (!task) {
        reply.status(404).send({ error: '任务不存在' });
  ...
  fastify.post('/tasks/:id/cancel', async (request: FastifyRequest<{ Params: { id: string }; Body: { reason?: string; operatorName?: string } }>, reply) => {
    try {
      const task = await prisma.commandTask.findUnique({ where: { id: request.params.id } });
      if (!task) {
        reply.status(404).send({ error: '任务不存在' });
  ...
  fastify.get('/tasks/:id/progresses', async (request: FastifyRequest<{ Params: { id: string } }>, reply) => {
  ...
  fastify.post('/tasks/:id/progresses', async (request: FastifyRequest<{ Params: { id: string }; Body: ProgressCreate }>, reply) => {
    try {
      const task = await prisma.commandTask.findUnique({ where: { id: request.params.id } });
      if (!task) {
        reply.status(404).send({ error: '任务不存在' });
      ...
      if (task.status === 'PENDING' && body.progress > 0) newStatus = 'IN_PROGRESS';
      let actualStart = task.actualStart;
      if (!actualStart && body.progress > 0) actualStart = now;

      const updatedTask = await prisma.commandTask.update({
        where: { id: request.params.id },
        data: {
          progress: body.progress,
          status: newStatus,
      ...
      const meta = getRequestMeta(request);
      await createOperationLog({
        targetType: TargetType.TASK_PROGRESS,
        targetId: progress.id,
        action: ActionType.TASK_PROGRESS_UPDATE,
        description: `任务进度回填：${task.taskNumber} - ${task.title}，进度 ${body.progress}%`,
        operator: body.reporterName || extractOperator(request),
        afterData: {
  ...
  fastify.get('/flows/stats', async () => {
    const allFlows = await prisma.flowRecord.findMany();
    const byAction: any = {};
    ...
    allFlows.forEach((f: any) => {
      ...
      if (f.createdAt >= sevenDaysAgo) last7Days++;
    });
  ...
  fastify.post('/flows', async (request: FastifyRequest<{ Body: FlowCreate }>, reply) => {
    try {
      ...
      const flow = await prisma.flowRecord.create({
        data: {
          taskId: body.taskId,
      ...
      const actionMap: any = {
        ASSIGN: ActionType.FLOW_ASSIGN,
        TRANSFER: ActionType.FLOW_TRANSFER,
        RECEIVE: ActionType.FLOW_RECEIVE,
        RETURN: ActionType.FLOW_RETURN,
      };

      const meta = getRequestMeta(request);
      await createOperationLog({
        targetType: TargetType.FLOW_RECORD,
        targetId: flow.id,
        action: actionMap[body.action] || ActionType.CREATE,
  ...
  fastify.get('/flows/:sourceType/:sourceId', async (request: FastifyRequest<{ Params: { sourceType: string; sourceId: string } }>, reply) => {
    const flows = await prisma.flowRecord.findMany({
      where: {
        sourceType: request.params.sourceType as any,
  ...
  fastify.get('/dashboard/overview', async () => {
    ...
    const activeTasks = await prisma.commandTask.findMany({
    ...
    const taskStats = {
      ...
      completed: await prisma.commandTask.count({ where: { status: 'COMPLETED' } }),
      overdue: allWarnings.filter((w) => w.type === 'overdue').length,
      nearDue: allWarnings.filter((w) => w.type === 'nearDue').length,
    };

    const recentFlows = await prisma.flowRecord.findMany({
      take: 10,
      orderBy: { actionTime: 'desc' },
      include: {
  ...
  fastify.get('/check-overdue', async () => {
    const now = new Date();
    const tasksToUpdate = await prisma.commandTask.findMany({
      where: {
        status: { in: ['PENDING', 'IN_PROGRESS'] },
        dueDate: { lt: now },
      },
    ...
    for (const task of tasksToUpdate) {
      await prisma.commandTask.update({
        where: { id: task.id },
        data: { status: 'OVERDUE' },
      });
