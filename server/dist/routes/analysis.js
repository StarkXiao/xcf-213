...
async function default_1(fastify) {
    fastify.get('/overview', async () => {
        ...
        const multiCasePersons = await prisma_1.default.casePerson.groupBy({
            by: ['personId'],
            _count: { caseId: true },
            having: { caseId: { _count: { gte: 2 } } },
        });
        const caseWithLocation = await prisma_1.default.case.count({
            where: { location: { not: null, not: '' } },
        });
        const evidenceWithHash = await prisma_1.default.evidence.count({
    ...
    fastify.get('/cross-case', async (request, reply) => {
        ...
        if (cTypes)
            caseFilter.caseType = { in: cTypes };
        const cStatuses = toArray(caseStatuses);
        if (cStatuses)
            caseFilter.status = { in: cStatuses };
        ...
        if (dims.includes('persons')) {
            ...
            casePersons.forEach(cp => {
                ...
                if (cp.role)
                    personCaseMap.get(key).roles.add(cp.role);
            });
        ...
        if (dims.includes('locations')) {
            ...
            if (allCaseIdsFromCluesAndEvidences.size > 0) {
                ...
                locationMap.forEach(entry => {
                    entry.caseIds.forEach((caseId) => {
                        if (!entry.cases.find((c) => c.id === caseId) && missingCaseMap.has(caseId)) {
        ...
        if (dims.includes('times')) {
            const casesWithTime = await prisma_1.default.case.findMany({
    ...
    fastify.get('/case-groups', async (request, reply) => {
        ...
        caseIds.forEach(id => {
            ...
            if (loc)
                g.locations.add(loc);
            if (caseMap.get(id))
                g.caseTypes.add(caseMap.get(id).caseType);
            const persons = caseToPersons.get(id);
            if (persons)
                persons.forEach(p => g.sharedPersons.set(p, (g.sharedPersons.get(p) || 0) + 1));
        });
        const personIds = new Set();
        groups.forEach(g => {
            g.sharedPersons.forEach((count, pid) => {
                if (count >= 2)
                    personIds.add(pid);
            });
        });
        let personDetails = [];
        if (personIds.size > 0) {
            personDetails = await prisma_1.default.person.findMany({
    ...
    fastify.get('/case-cluster/:caseId', async (request, reply) => {
        const { caseId } = request.params;
        const baseCase = await prisma_1.default.case.findUnique({
            where: { id: caseId },
            include: {
        ...
        const allCasePersons = await prisma_1.default.casePerson.findMany({
        ...
        if (baseLocation) {
            const otherCases = await prisma_1.default.case.findMany({
                where: {
                    id: { not: caseId },
        ...
        caseRelationMap.forEach((entry, cid) => {
            ...
            if (entry.sharedLocation)
                score += 25;
            if (entry.sharedPersons.length > 0 && entry.sharedLocation)
                score += 20;
